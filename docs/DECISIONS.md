...
## 2026-06-12 — Online multiplayer: server-authoritative on Vercel Functions + Supabase

Decision: Add online 2-player co-op. The pure engine (`src/game/`) runs server-side in Vercel Serverless Functions under `api/`, using a Supabase `service_role` key; Supabase Postgres stores game state. Clients authenticate via Supabase anonymous sign-in, send only moves, and render a per-player redacted view. Realtime uses a `game_events` tick table (no state diffused); clients refetch their redacted view on each tick. Local pass-and-play is untouched.
Context: User asked to turn the hotseat game into online multiplayer with a home screen (local/online), a lobby (nickname, create/join), and a 3-char room code, reusing their existing Supabase DB pattern (3 generic tables, RLS, redacted views, pg_cron cleanup).
